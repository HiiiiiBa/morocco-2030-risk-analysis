...
export default function TangerMap() {
  ...
  const loadZoneComments = async (zoneId: string) => {
    try {
      const response = await fetch(`${API_URL}/cities/tanger/districts/${zoneId}/comments`)
      if (response.ok) {
  ...
  const addComment = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/cities/tanger/districts/${selectedZone.id}/comments`, {
        method: "POST",
        headers: {
