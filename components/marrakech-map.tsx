...
const marrakechZones: MarrakechZone[] = [
  {
    id: "gueliz",
    name: "Guéliz",
    lat: 31.6337,
...
export default function MarrakechMap() {
  ...
  const [selectedZone, setSelectedZone] = useState<MarrakechZone | null>(null)
  ...
  const handleSearch = () => {
    ...
    const foundZone = marrakechZones.find((zone) => 
  ...
  const addZoneMarkers = useCallback(() => {
    ...
    console.log("Starting to add markers for", marrakechZones.length, "zones")
    ...
    console.log("Successfully added", marrakechZones.length, "markers to the map")
  }, [])
    ...
    if (typeof window !== "undefined" && mapRef.current && !mapInstanceRef.current && isClient) {
      ...
      script.onload = () => {
        ...
        const mapInstance = L.map(mapRef.current, {
          center: [31.6226, -7.9899], // Centré sur Marrakech-Médina
          zoom: 12,
          zoomControl: true,
    ...
    return (
      <div className="w-full h-96 bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Chargement de la carte de Marrakech...</p>
        </div>
      </div>
    )
  }
